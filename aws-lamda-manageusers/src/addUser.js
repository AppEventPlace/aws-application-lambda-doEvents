...
exports.addUser = async (event) => {
    ...
    try {
        ...
        const { name, lastName, date, phone, email, user, password, fotoPerfilBase64 } = JSON.parse(event.body);
        ...
        try {
            ...
            throw new Error("Error al verificar el número de teléfono en la base de datos");
        }
        ...
        if (error.message === "Todos los campos son obligatorios" || error.message === "El usuario con este email ya existe" || error.message === "El usuario con este número de teléfono ya existe") {
            errorMessage = error.message;
            statusCode = 400;
        } else if (error.message === "Error al verificar el email en la base de datos" || error.message === "Error al verificar el número de teléfono en la base de datos") {
            errorMessage = error.message;
            statusCode = 500;
        }
